...
def solve(G, args):
    ...
    opt_model.solve(solver=plp.apis.GUROBI(mip=True, msg=msg,
                                       timeLimit=time_limit,
                                       ImproveStartTime=time_limit*0.9,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--time_limit", type=int, nargs="?", action="store", default=10, help="Time limit in minutes")
    ...
    for idx,graph_path in enumerate(args.input.rglob(f"*_{'weighted' if args.weighted else 'unweighted'}.graph")):
        ...
        solu, sum, status, total_time, explore_time = solve(G, args)
